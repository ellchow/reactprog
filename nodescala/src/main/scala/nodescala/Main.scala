...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val myServerSubscription = myServer.start("/test")(_.map(_.toString).iterator)
    ...
    val terminationRequested: Future[String] = Future.any(List(Future.delay(10 seconds).map(_ => ""), userInterrupted))
